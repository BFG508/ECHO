...
export function renderCreate(root) {
    ...
    burnCopy.append(element('strong', '', 'Burn after reading'), element('small', '', 'Delete the encrypted payload after the first authorized reveal.'));
...
function renderCreated(root, secrets, burn, expirySeconds) {
    clear(root);
    const shell = element('section', 'card');
    ...
    const warning = statusBox('info', `The master secret lives only after the # in this link. ECHO never uploads it. Expires in ${EXPIRATIONS.get(expirySeconds) ?? 'the selected interval'}.`);
...
export function renderOpen(root) {
    ...
    shell.append(element('p', 'eyebrow', 'Encrypted echo'), element('h1', 'title', 'A private message is waiting'), element('p', 'lead', 'Nothing is fetched until you choose to reveal it. This avoids consuming one-time messages through link previews.'));
...
function renderMessage(root, message, burned, expiresAt) {
    clear(root);
    const shell = element('section', 'card');
    shell.append(element('p', 'eyebrow', burned ? 'Opened and burned' : 'Decrypted locally'), element('h1', 'title', 'The echo says'));
