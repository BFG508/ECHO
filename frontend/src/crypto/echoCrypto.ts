...
import type { EncryptedEcho, ShareSecrets } from '../types/echo.js';
...
function idBytesFor(id: string): Uint8Array {
...
async function importMasterSecret(secretBytes: Uint8Array): Promise<CryptoKey> {
...
async function deriveAesKey(masterSecret: CryptoKey, idBytes: Uint8Array): Promise<CryptoKey> {
...
async function deriveAccessProof(masterSecret: CryptoKey, idBytes: Uint8Array): Promise<Uint8Array> {
...
export function plaintextByteLength(message: string): number {
...
export async function encryptEcho(message: string): Promise<EncryptedEcho> {
...
export async function decryptEcho(
  id: string,
...
export async function deriveAccessToken(id: string, encodedSecret: string): Promise<string> {
  const masterSecretBytes = base64UrlToBytes(encodedSecret);
  const masterSecret = await importMasterSecret(masterSecretBytes);
...
export function formatShareFragment(secrets: ShareSecrets): string {
  return `#/open/${secrets.id}.${secrets.key}`;
}

export function parseShareFragment(hash: string): ShareSecrets | null {
  ...
  try {
    if (base64UrlToBytes(id).byteLength !== ID_BYTES) return null;
    if (base64UrlToBytes(key).byteLength !== MASTER_SECRET_BYTES) return null;
  } catch {
    return null;
  }
