...
export function formatRemaining(seconds) {
    ...
    if (safe < 60)
        return formatPart(safe, 'second');
    const minutes = Math.floor(safe / 60);
    if (minutes < 60)
        return `${formatPart(minutes, 'minute')} ${formatPart(safe % 60, 'second')}`;
    const hours = Math.floor(minutes / 60);
    if (hours < 24)
        return `${formatPart(hours, 'hour')} ${formatPart(minutes % 60, 'minute')}`;
    const days = Math.floor(hours / 24);
    return `${formatPart(days, 'day')} ${formatPart(hours % 24, 'hour')}`;
}
export function startCountdown(element, expiresAtSeconds) {
    const update = () => {
